...
/**
 * This represents an immutable path between two general items, particularly
...
public class Path<E> implements Iterable<Path<E>.Segment> {
    ...
    // AF(this) =
    //      first item in the path => start
    //      each "step" along the path between items => elements of list path, where
    //          path.get(0) is the first step from the start to an intermediate spot, and
    //          path.get(path.size() - 1) is the final step from an intermediate spot to the end
    ...
    /**
     * The ordered sequence of segments representing a path between items.
     */
    ...
    /**
     * Appends a new single segment to the end of this path, originating at the current last spot
     ...
     * segmentCost}.
     *
     * @param newEnd      The spot being added at the end of the segment being appended to this path
     ...
     * both paths are empty, they are only equal if their starts are equal.
     *
    ...
    /**
     * Segment represents a single segment as part of a longer, more complex path between items.
    ...
    public class Segment {
         ...
         * @throws NullPointerException     if either the start or the end is null.
